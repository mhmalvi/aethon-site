...
export function ProductDetailHero({ featureIndex }: { featureIndex: number }) {
  const feature = PRODUCTS.features[featureIndex];
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          ...
          style={{
            top: "20%",
            right: "10%",
            background: featureAccents[featureIndex].color,
            opacity: 0.08,
            filter: "blur(120px)",
          }}
        />
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto w-full">
        ...
        <motion.p
          ...
          style={{ color: featureAccents[featureIndex].color, opacity: 0.7 }}
        >
          {feature.tagline}
        </motion.p>
        ...
        <div className="space-y-2 mb-8">
          <motion.div
            ...
            style={{
              background: `linear-gradient(90deg, ${featureAccents[featureIndex].color}99, transparent)`,
            }}
          />
          <motion.div
            initial={{ scaleX: 0 }}
...
export function ProductDetailContent({ featureIndex }: { featureIndex: number }) {
  const feature = PRODUCTS.features[featureIndex];
  const accent = featureAccents[featureIndex];
  const Icon = iconMap[featureIndex];
  const ref = useRef<HTMLDivElement>(null);
